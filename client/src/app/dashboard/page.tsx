...
//   faMagnifyingGlass, faChevronLeft, faChevronRight, faHeart
...
// function SearchAndPagination({ searchQuery, setSearchQuery, totalPages, currentPage, onPageChange, placeholder = 'সার্চ করুন...' }: SearchAndPaginationProps) {
//   return (
...
//   const [stats, setStats] = useState({ totalSales: 0, totalRevenue: 0, productsListed: 0, walletBalance: 0 });
//   const [stats, setStats] = useState({ 
//     totalSales: 0, 
//     totalRevenue: 0, 
//     productsListed: 0, 
//     walletBalance: 0,
//     affiliateBalance: 0,      // 🔥 নতুন
//     referralCode: ''          // 🔥 নতুন
//   });
...
//   // 🔥 সার্চ ও পেজিনেশন স্টেট (প্রোডাক্ট এবং উইথড্রের জন্য আলাদা)
...
//       try {
//         const { data: profile } = await supabase.from('profiles').select('wallet_balance, name, role').eq('id', user.id).maybeSingle();

//           // যদি referral_code না থাকে, তবে একটি জেনারেট করে আপডেট করে দিন
...
//           affiliateBalance: profile?.affiliate_balance || 0, // 🔥 সেট করা হলো
...
//         product_id: requestUpdateProduct.id,
//         seller_id: user.id,
...
//       setFeedback({ isOpen: true, type: 'success', message: 'ফাইল আপডেটের অনুরোধ সফলভাবে পাঠানো হয়েছে। অ্যাডমিন শীঘ্রই পর্যালোচনা করবেন।' });
...
//   // 🔥 চার্টের জন্য ডেটা প্রস্তুত করা (প্রতিটি প্রোডাক্টের বিক্রির সংখ্যা)
//   const chartData = products.map(p => ({
//     name: p.title.length > 15 ? p.title.substring(0, 15) + '...' : p.title,
//     বিক্রি: p.real_download_count || 0,
//     fullTitle: p.title
//   })).sort((a, b) => b.বিক্রি - a.বিক্রি).slice(0, 5); // শীর্ষ ৫টি প্রোডাক্ট
...
//             </Link>
//             <Link href="/dashboard/sales" className="flex-1 sm:flex-none"><Button variant="outline" size="md" icon={faChartLine} fullWidth>বিক্রির হিস্টরি</Button></Link>
//             <Link href="/dashboard/upload" className="flex-1 sm:flex-none"><Button variant="primary" size="md" icon={faPlus} fullWidth>নতুন প্রোডাক্ট আপলোড</Button></Link>
//           </div>
//         </div>
...
//         <div className="bg-gradient-to-br from-purple-600 to-indigo-700 rounded-2xl border border-purple-500/30 p-6 text-white shadow-lg">
//           <div className="flex items-start justify-between mb-4">
//             <div>
//               <h3 className="text-xl font-bold mb-1">রেফারেল প্রোগ্রাম</h3>
//               <p className="text-purple-100 text-sm">বন্ধুদের ইনভাইট করুন এবং প্রতিটি বিক্রিতে ১০% কমিশন পান!</p>
//             </div>
...
//             <p className="text-xs text-purple-200 mb-1">আপনার ইউনিক রেফারেল লিংক:</p>
//             <div className="flex items-center gap-2">
//               <code className="flex-1 bg-black/20 px-3 py-2 rounded-lg text-sm font-mono truncate">
//                 {typeof window !== 'undefined' ? `${window.location.origin}/register?ref=${stats.referralCode}` : 'Loading...'}
//               </code>
//               <button 
//                 onClick={() => {
...
//                 }}
//                 className="px-4 py-2 bg-white text-purple-700 rounded-lg text-sm font-bold hover:bg-purple-50 transition-colors"
//               >
//                 কপি
//               </button>
//             </div>
...
//             <span className="text-purple-100 text-sm">মোট অ্যাফিলিয়েট আয়</span>
...
//         {products.length > 0 && (
//           <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 mb-10">
//             <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-6 font-bangla flex items-center gap-2">
//               <FontAwesomeIcon icon={faChartLine} className="text-indigo-500" />
//               শীর্ষ ৫টি বিক্রিত প্রোডাক্ট
//             </h2>
...
//                     }}
//                     formatter={(value: number, name: string, props: any) => [`${value} টি`, 'বিক্রি']}
//                     labelFormatter={(label: string, props: any) => props?.payload?.[0]?.payload?.fullTitle || label}
//                   />
//                   <Bar dataKey="বিক্রি" radius={[6, 6, 0, 0]}>
...
//             </div>
//           ) : (
//             /* 🔥 overflow-x-auto এবং min-w-[600px] যোগ করা হয়েছে মোবাইল স্ক্রল এর জন্য */
...
//             setSearchQuery={setWithdrawalSearch}
//             totalPages={withdrawalsData.totalPages}
//             currentPage={withdrawalPage}
//             onPageChange={setWithdrawalPage}
//             placeholder="পেমেন্ট মাধ্যম বা নম্বর দিয়ে সার্চ করুন..."
//           />
...
//               <strong>প্রোডাক্ট:</strong> {requestUpdateProduct?.title}
//             </p>
//             <p className="text-xs text-amber-700 dark:text-amber-400 mt-1 font-bangla">
//               ⚠️ নোট: অ্যাডমিন আপনার অনুরোধ পর্যালোচনা করে অনুমোদন করলেই শুধু ফাইলটি আপডেট হবে।
//             </p>
//           </div>
...
//               className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:border-indigo-500 outline-none resize-none font-bangla" 
//               placeholder="যেমন: PDF-এ একটি টাইপো ঠিক করতে চাই / জিপ ফাইলের ভেতরে একটি ছোট ফাইল রিপ্লেস করতে চাই..."
//             />
//           </div>
...
//               type="file" 
//               ref={fileInputRef}
//               onChange={(e) => e.target.files && setUpdateFile(e.target.files[0])}
//               className="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:border-indigo-500 outline-none font-bangla" 
//             />
//           </div>
...
//               className="w-full px-3 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none font-bangla transition-all" 
//               placeholder={`সর্বনিম্ন ১০০, সর্বোচ্চ ৳${stats.walletBalance.toFixed(2)}`}
//             />
//           </div>
...
//           <div>
//             <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1 font-bangla">
//               {withdrawForm.payment_method === 'bank' ? 'Bank Account Number' : 'মোবাইল নম্বর'}
//             </label>
//             <input 
//               type="text" 
//               required 
//               value={withdrawForm.payment_number} 
//               onChange={(e) => setWithdrawForm({...withdrawForm, payment_number: e.target.value})} 
//               className="w-full px-3 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 outline-none font-bangla transition-all" 
//               placeholder={withdrawForm.payment_method === 'bank' ? 'Account No: 1234567890' : '01XXXXXXXXX'}
//             />
//           </div>

//           <div className="flex gap-3 pt-2">
//             <Button type="button" variant="outline" fullWidth onClick={() => setShowWithdrawModal(false)}>বাতিল</Button>
//             <Button type="submit" variant="primary" fullWidth loading={isProcessing}>
//               {stats.walletBalance >= 100 ? 'রিকোয়েস্ট পাঠান' : 'ব্যালেন্স অপর্যাপ্ত'}
//             </Button>
//           </div>
//         </form>
//       </Modal>

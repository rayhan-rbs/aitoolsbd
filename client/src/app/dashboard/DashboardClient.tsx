  ...
  return (
    <div className="mb-6 flex flex-col sm:flex-row items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-800">
      <div className="relative w-full sm:w-80">
        ...
        <input type="text" placeholder="সার্চ করুন..." value={searchQuery} onChange={(e) => { setSearchQuery(e.target.value); onPageChange(1); }} className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none font-bangla text-sm transition-all" />
      </div>
      {totalPages > 1 && (
...
export default function DashboardClient() {
  ...
  const [stats, setStats] = useState({ totalSales: 0, totalRevenue: 0, productsListed: 0, walletBalance: 0, affiliateBalance: 0, referralCode: '' });
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      ...
      try {
        ...
        const { data: productsData } = await supabase.from('products').select('id, title, price, description, category, preview_url, download_count, is_approved, created_at, is_deleted').eq('seller_id', user.id).order('created_at', { ascending: false });
        const allProducts = productsData || [];
        const activeProducts = allProducts.filter(p => p.is_deleted !== true);
        let validSalesCount = 0;
        let actualRevenue = 0;
        const { data: ordersData } = await supabase.from('orders').select('id, buyer_id, seller_earning, status, current_downloads, product_id').eq('seller_id', user.id);
        if (ordersData) {
        ...
        const productsWithRealDownloads = await Promise.all(
          activeProducts.map(async (product) => {
            const { data: validOrders } = await supabase.from('orders').select('id, current_downloads').eq('product_id', product.id).eq('status', 'completed').neq('buyer_id', user.id);
        ...
        const { data: withdrawalData } = await supabase.from('withdrawals').select('*').eq('seller_id', user.id).order('created_at', { ascending: false });
        setWithdrawals(withdrawalData || []);
        setStats({ totalSales: validSalesCount, totalRevenue: actualRevenue, productsListed: activeProducts.length, walletBalance: profile?.wallet_balance || 0, affiliateBalance: profile?.affiliate_balance || 0, referralCode: finalReferralCode || '' });
  ...
  const getFilteredAndPaginatedData = (data: any[], searchFields: string[], query: string, page: number) => {
    const filtered = data.filter(item => searchFields.some(field => {
  ...
  const handleWithdraw = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (amount > stats.walletBalance) { setFeedback({ isOpen: true, type: 'error', message: 'অপর্যাপ্ত ব্যালেন্স!' }); setIsProcessing(false); return; }
      if (amount < 100) { setFeedback({ isOpen: true, type: 'error', message: 'সর্বনিম্ন উইথড্র পরিমাণ ১০০ টাকা।' }); setIsProcessing(false); return; }
      const { error } = await supabase.from('withdrawals').insert({ seller_id: user.id, amount: amount, payment_method: withdrawForm.payment_method, payment_number: withdrawForm.payment_number, status: 'pending' });
      if (error) throw error;
  ...
  const handleRequestFileUpdate = async () => {
    ...
    try {
      ...
      const { error: dbError } = await supabase.from('product_update_requests').insert({ product_id: requestUpdateProduct.id, seller_id: user.id, new_file_url: publicUrl, reason: updateReason, status: 'pending' });
      if (dbError) throw dbError;
  ...
  // 🔥 লক্ষ্য করুন: এখানে আর <Suspense> নেই। শুধু মূল JSX ফেরত দেওয়া হচ্ছে।
  return (
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between mb-10 gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            <Link href="/dashboard/wishlist" className="flex-1 sm:flex-none"><Button variant="outline" size="md" icon={faHeart} fullWidth>আমার Wishlist</Button></Link>
            <Link href="/dashboard/buyer" className="flex-1 sm:flex-none"><Button variant="outline" size="md" icon={faShoppingBag} fullWidth>ক্রেতা ড্যাশবোর্ড</Button></Link>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 hover:shadow-lg hover:shadow-slate-200/50 dark:hover:shadow-slate-900/50 transition-all duration-300 hover:-translate-y-1">
            ...
            <div>
              ...
              <button onClick={() => setShowWithdrawModal(true)} disabled={stats.walletBalance < 100} className="w-full py-2 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-300 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-lg transition-all font-bangla">টাকা তুলুন</button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-600 to-indigo-700 rounded-2xl border border-purple-500/30 p-6 text-white shadow-lg mb-10">
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-4 mb-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/?ref=${stats.referralCode}`); setFeedback({ isOpen: true, type: 'success', message: 'রেফারেল লিংক কপি হয়েছে!' }); }} className="px-4 py-2 bg-white text-purple-700 rounded-lg text-sm font-bold hover:bg-purple-50 transition-colors font-bangla">কপি</button>
            </div>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden p-6 mb-6">
          ...
          {productsData.paginated.length === 0 ? (
            <div className="p-12 text-center">
              ...
              {!products.length && (<Link href="/dashboard/upload"><Button variant="primary" size="sm" icon={faPlus}>প্রথম প্রোডাক্ট আপলোড করুন</Button></Link>)}
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                  {withdrawalsData.paginated.map((w) => (
                    <tr key={w.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="px-4 py-4">
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold font-bangla ${w.status === 'approved' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400' : w.status === 'rejected' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400' : 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400'}`}>
